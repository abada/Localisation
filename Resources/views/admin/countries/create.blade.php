...
@section('content-header')
    <h1>
        {{ trans('localisation::countries.title.create country') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.localisation.country.index') }}">{{ trans('localisation::countries.title.countries') }}</a></li>
        <li class="active">{{ trans('localisation::countries.title.create country') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.localisation.country.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-10">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('localisation::admin.countries.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
